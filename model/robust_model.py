from model.robust_encoder import RobustEncoder
from model.robust_decoder import RobustDecoder
...
class RobustModel:
    """Model with robust dual-domain watermarking"""
    def __init__(self, image_size, watermark_length, device):
        ...
        configuration = Configuration(
            ...
            decoder_loss=1.5,  # Increased for dual-domain
            encoder_loss=0.7,
            adversarial_loss=1e-3,
            enable_fp16=False
        )
        self.encoder = RobustEncoder(configuration).to(device)
        self.decoder = RobustDecoder(configuration).to(device)